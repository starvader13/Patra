...
const ColoredTemplate = ({name, description, age, email, hobbies, linkedin, twitter, customUrl, url}: (CreateCard & {url: string})) => {
    ...
    return <div className={"flex flex-col justify-start items-center border-none rounded-xl w-[50%] my-8 bg-gradient-to-tr from-emerald-200 to-red-200 gap-6 shadow-lg shadow-green-200"}>
        {
        ...
        {
            url ? <div
                className={"mx-6 mb-6 mt-6 flex justify-center rounded-xl bg-gradient-to-tl from-amber-200 to-violet-500 w-1/5"}>
...

        {
            hobbiesArray.length !==0 && hobbiesArray[0] !== "" ? <>
                    <div className={"p-1 bg-gray-300 rounded-xl w-[90%]"}></div>
        ...
        {
            customUrl ? <>
                <div className="shadow-lg bg-gray-200 hover:text-3xl text-2xl p-3 shadow-gray-800 font-mono flex justify-center items-center font-bold text-gray-900 underline underline-offset-4 mb-8">
